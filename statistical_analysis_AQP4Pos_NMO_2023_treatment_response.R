...
savepath<-"D:/ZZZ/Manuscripts/Subtyping/Sustain_results/Results/AQP4_NMOSD_0411"
setwd(savepath)
...
#colnames(Data)[2]<-'Group_subtype'
#Data[Data$Group=='MS'&Data$ml_subtype==0,"Group_subtype"]='A0';
#Data[Data$Group=='MS'&Data$ml_subtype==1,"Group_subtype"]='A1';
#Data[Data$Group=='MS'&Data$ml_subtype==3,"Group_subtype"]='A2';

Data[Data$Group=='AQP4Pos_NMOSD'&Data$ml_stage==0,"Group_subtype"]='AQP4+NA';
Data[Data$Group=='AQP4Pos_NMOSD'&Data$ml_subtype==0&Data$ml_stage>0,"Group_subtype"]='AQP4+C';
Data[Data$Group=='AQP4Pos_NMOSD'&Data$ml_subtype==1&Data$ml_stage>0,"Group_subtype"]='AQP4+SC';
Data[Data$Group=='AQP4Pos_NMOSD'&Data$ml_subtype==2&Data$ml_stage>0,"Group_subtype"]='AQP4+CE';
#Data[Data$Group=='AQP4Neg_NMOSD'&Data$ml_subtype==0,"Group_subtype"]='C0';
#Data[Data$Group=='AQP4Neg_NMOSD'&Data$ml_subtype==1,"Group_subtype"]='C1';
#Data[Data$Group=='MOGAD'&Data$ml_subtype==0,"Group_subtype"]='D0';
#Data[Data$Group=='MOGAD'&Data$ml_subtype==1,"Group_subtype"]='D1';


Data$Group_subtype<-factor(Data$Group_subtype,levels=c('AQP4+NA','AQP4+C','AQP4+SC','AQP4+CE'))

#write.csv(Data,'AQP4Pos_NMOSD_subtype_data.csv');


Data=read.csv('D:/ZZZ/Manuscripts/Subtyping/Sustain_results/Results/AQP4_NMOSD_0411/AQP4Pos_NMOSD_subtype_data.csv',header = TRUE);
...
#???????߷???
setwd('D:/ZZZ/ManuscriptsSubtyping/Sustain_results/Results/AQP4_NMOSD_0411')
var1<-c('AQP4+NA','AQP4+C','AQP4+SC','AQP4+CE')
var2<-c('DMT','Immunosuppressant')


Data$Treatment<-factor(Data$Treatment,levels=c('DMT','Immunosuppressant'))
...
for(i in var1)
{
  tem_data=Data1[Data1$Group_subtype==i,]
  
  res<-pairwise_survdiff(Surv(Followup_time_years,EDSS_Progress)~Treatment,data = tem_data)
  
  print(res)
  tem_frame<-res$p.value
  
  write.csv(tem_frame,paste0(i,'_treatment_response_EDSS_Progress.csv'));
  
  fit<-survfit(Surv(tem_data$Followup_time_years,tem_data$EDSS_Progress)~tem_data$Treatment,data=tem_data)
  
  tem<-summary(fit)
  
  write.csv(tem$table,paste0(i,'_treatment_response_EDSS_Progress_median_surrive_time.csv'));
  
  pdf(file=paste0(i,'_treatment_response_EDSS_Progress_Surrival_curve.pdf'),width = 8,height=8)
...
for(i in var2)
{
  tem_data=Data1[Data1$Treatment==i,]
  
  res<-pairwise_survdiff(Surv(Followup_time_years,EDSS_Progress)~Group_subtype,data = tem_data)
  ...
  fit<-survfit(Surv(tem_data$Followup_time_years,tem_data$EDSS_Progress)~tem_data$Group_subtype,data=tem_data)
  
  tem<-summary(fit)
  
  write.csv(tem$table,paste0(i,'_treatment_response_EDSS_Progress_median_surrive_time.csv'));
  
  pdf(file=paste0(i,'_treatment_response_EDSS_Progress_Surrival_curve.pdf'),width = 12,height=8)
  p1<-ggsurvplot(fit,pval=TRUE,conf.int = FALSE,risk.table = TRUE,risk.table.y.text.col=TRUE,risk.table.col="strata",)
  print(p1)
  dev.off()
...
Data1$Group_subtype<-factor(Data1$Group_subtype,levels=c('AQP4+NA','AQP4+C','AQP4+SC','AQP4+CE'))

Data1$Followup_relapse<-as.factor(Data1$Followup_relapse)

tem_Data1<-Data1[Data1$Treatment=='DMT',]
tem_Data1$Group_subtype<-factor(tem_Data1$Group_subtype,levels=c('AQP4+NA','AQP4+C','AQP4+SC','AQP4+CE'))
tem_Data1$Followup_relapse<-factor(tem_Data1$Followup_relapse,levels=c('1','0'))

mytable <- xtabs(~ Group_subtype+Followup_relapse, data=tem_Data1)

mytable
# install.packages("rcompanion")
library(rcompanion)
pairwiseNominalIndependence(mytable, fisher = T,gtest = FALSE, chisq = TRUE, method = "fdr")

value<-mytable[,2]/(mytable[,2]+mytable[,1])
barplot(value,width=0.3,ylim=c(0,1))

write.csv(mytable,paste0('NMOSD_count_Followup_relapse.csv'));

pdf(file=paste0('NMOSD_count_Followup_relapse.pdf'),width = 5,height=3)
ggplot(tem_Data1, aes(x = Group_subtype,color=Followup_relapse)) +
  geom_bar(aes(fill = Followup_relapse),position = "stack",width=0.3) +
  theme(legend.position = "top")+ylim(0,35)+
  theme_classic()
dev.off()


chisq.test(tem_Data1$Group_subtype, tem_Data1$Followup_relapse)$expected

mulchi<-chisq.test(tem_Data1$Group_subtype, tem_Data1$Followup_relapse)
...
Data1$EDSS_Progress<-as.factor(Data1$EDSS_Progress)

tem_Data1<-Data1[Data1$Treatment=='DMT',]
tem_Data1$Group_subtype<-factor(tem_Data1$Group_subtype,levels=c('AQP4+NA','AQP4+C','AQP4+SC','AQP4+CE'))
tem_Data1$EDSS_Progress<-factor(tem_Data1$EDSS_Progress,levels=c('1','0'))

mytable <- xtabs(~ Group_subtype+EDSS_Progress, data=tem_Data1)

mytable
pairwiseNominalIndependence(mytable, fisher = T,gtest = FALSE, chisq = TRUE, method = "fdr")


write.csv(mytable,paste0('NMOSD_count_EDSS_Progress.csv'));

pdf(file=paste0('NMOSD_count_EDSS_Progress.pdf'),width = 5,height=3)
ggplot(tem_Data1, aes(x = Group_subtype,color=EDSS_Progress)) +
  geom_bar(aes(fill = EDSS_Progress),position = "stack",width=0.3) +
  theme(legend.position = "top")+ylim(0,35)+
  theme_classic()
dev.off()


# Data1[Data1$Followup_relapse==0,'Followup_relapse_rate']=Data1[Data1$Followup_relapse==0,'Followup_relapse']/Data1[Data1$Followup_relapse==0,'Followup_relapse']
# Data1[Data1$Followup_relapse>0,'Followup_relapse_rate']=1


# Data1<-Data
# Data1<-Data1[!(is.na(Data1$Followup_relapse)|Data1$Followup_relapse=="[]"|Data1$Followup_relapse==""|Data1$Followup_relapse=="NaN"),]
# 
# Data1[,'Followup_relapse_rate']=Data1$Followup_relapse/Data1$Followup_time_years
# 
# #Data1$ml_stage<-as.factor(Data1$ml_stage)
# Data1$Sex_F1_M0<-as.factor(Data1$Sex_F1_M0)
# 
...
# tem_Data1$Group_subtype<-factor(tem_Data1$Group_subtype,levels=c('AQP4+NA','AQP4+C','AQP4+SC','AQP4+CE'))
# 
# pdf(file=paste0('NMOSD_count_Followup_relapse.pdf'),width = 5,height=5)
# ggplot(tem_Data1, aes(x = Group_subtype,y=Followup_relapse_rate)) +
#   geom_boxplot() +
...
# 
# 
# ggboxplot(tem_Data1, x = "Group_subtype", y = "Followup_relapse_rate",
...
ggplot(tem_Data1, aes(x = Group_subtype,color=Followup_relapse)) +
  geom_bar(aes(fill = Followup_relapse),width=0.3,position = "dodge") +
  theme(legend.position = "top")+
  theme_bw()+
  geom_text(aes(label=..count..),stat = 'count',color ="black")+


ggplot(tem_Data1, aes(x = Group_subtype,color=Followup_relapse)) +
  geom_bar(aes(fill = Followup_relapse),stat = "count",position = "fill",width=0.3) +
  theme(legend.position = "top")+
  theme_bw()+
...
scale_y_continuous(labels = scales::percent)ggplot(tem_Data1, aes(y = Group_subtype)) +
  geom_bar(aes(fill = Followup_relapse), position = position_stack(reverse = TRUE)) +
  theme(legend.position = "top")
...
for(i in var1)
{
  ...
  res<-pairwise_survdiff(Surv(Followup_time_years,Followup_relapse)~Treatment,data = tem_data)
  
  print(res)
  tem_frame<-res$p.value
  
  write.csv(tem_frame,paste0(i,'_treatment_response_Followup_relapse.csv'));
  
  fit<-survfit(Surv(tem_data$Followup_time_years,tem_data$Followup_relapse)~tem_data$Treatment,data=tem_data)
  
  tem<-summary(fit)
  
  write.csv(tem$table,paste0(i,'_treatment_response_Followup_relapse_median_surrive_time.csv'));
  
  pdf(file=paste0(i,'_treatment_response_Followup_relapse_Surrival_curve.pdf'),width = 8,height=8)
  p1<-ggsurvplot(fit,pval=TRUE,conf.int = FALSE,risk.table = TRUE,risk.table.y.text.col=TRUE,risk.table.col="strata",)
  print(p1)
  dev.off()
...
for(i in var2)
{
  ...
  res<-pairwise_survdiff(Surv(Followup_time_years,Followup_relapse)~Group_subtype,data = tem_data)
  ...
  fit<-survfit(Surv(tem_data$Followup_time_years,tem_data$Followup_relapse)~tem_data$Group_subtype,data=tem_data)
  
  tem<-summary(fit)
  
  write.csv(tem$table,paste0(i,'_treatment_response_Followup_relapse_median_surrive_time.csv'));
  
  pdf(file=paste0(i,'_treatment_response_Followup_relapse_Surrival_curve.pdf'),width = 12,height=8)
  p1<-ggsurvplot(fit,pval=TRUE,conf.int = FALSE,risk.table = TRUE,risk.table.y.text.col=TRUE,risk.table.col="strata",)
  print(p1)
  dev.off()
...
#   tem_data=Data1[Data1$Group_subtype==i,]
#   
#   res<-pairwise_survdiff(Surv(Followup_time_years,SPMS_conversion)~Treatment,data = tem_data)
#   
#   print(res)
#   tem_frame<-res$p.value
#   
#   write.csv(tem_frame,paste0(i,'_treatment_response_SPMS_conversion.csv'));
#   
#   fit<-survfit(Surv(tem_data$Followup_time_years,tem_data$SPMS_conversion)~tem_data$Treatment,data=tem_data)
#   
#   tem<-summary(fit)
#   
#   write.csv(tem$table,paste0(i,'_treatment_response_SPMS_conversion_median_surrive_time.csv'));
#   
#   pdf(file=paste0(i,'_treatment_response_SPMS_conversion_Surrival_curve.pdf'),width = 12,height=8)
#   p1<-ggsurvplot(fit,pval=TRUE,conf.int = FALSE,risk.table = TRUE,risk.table.y.text.col=TRUE,risk.table.col="strata",)
#   print(p1)
#   dev.off()
#   
...
#   tem_data=Data1[Data1$Treatment==i,]
#   
#   res<-pairwise_survdiff(Surv(Followup_time_years,SPMS_conversion)~Group_subtype,data = tem_data)
#   
#   print(res)
#   
#   tem_frame<-res$p.value
...
#   write.csv(tem_frame,paste0(i,'_treatment_response_SPMS_conversion.csv'));
#   
#   fit<-survfit(Surv(tem_data$Followup_time_years,tem_data$SPMS_conversion)~tem_data$Group_subtype,data=tem_data)
#   
#   tem<-summary(fit)
#   
#   write.csv(tem$table,paste0(i,'_treatment_response_SPMS_conversion_median_surrive_time.csv'));
...
#   
#   
#   fit<-lm(tem_var~Group_subtype,data=tem)
#   res0<-summary(fit)
#   
#   res1<-kruskal.test(tem_var~Group_subtype,data=tem)
#   res2<-with(data=tem,
#              pairwise.wilcox.test(x=tem_var,g=Group_subtype,p.adjust.method = "none")
#   )
#   
...
#   
#   # 
#   # Results[i,1]<-res0$coefficients[2,4]
#   # Results[i,2]<-res0$coefficients[3,4]
#   # Results[i,3]<-res0$coefficients[4,4]
#   # # Results[i,4]<-res2$p.value[2,1];
#   # # Results[i,5]<-res2$p.value[3,1];
#   # Results[i,6]<-str[i];
#   
...
# #one vs all others
# 
# G=c('AQP4+C','AQP4+SC','AQP4+CE')
# 
# 
# for(gg in G)
# {
#   tem_Data<-Data[Data$Group_subtype!='AQP4+NA',];
#   
...
# 
# 
# my_comparisons<-list(c('AQP4+NA','AQP4+C'),
#                      c('AQP4+NA','AQP4+SC'),
#                      c('AQP4+NA','AQP4+CE'),
#                    
#                      c('AQP4+C','AQP4+SC'),
...
# 
# 
# my_comparisons1<-list(c('AQP4+C','AQP4+SC'),
#                       c('AQP4+C','AQP4+CE'),
#   
#                       c('AQP4+SC','AQP4+CE'))
# 
...
# library(ggpmisc)
# 
# LETTERS<-c('AQP4+NA','AQP4+C','AQP4+SC','AQP4+CE')
# NG<-length(LETTERS)
...
#     
#     scale_y_continuous(#expand = c(0, 0),#?趨x????y???Ľ?????
#       
#       name =Ass_var[i],#?趨y??????
#       
#       #breaks=seq(0,50,10),#?趨Y???????ݼ???
#       
#       #limits = c(10,50) #?趨Y??????????????
#     )+
#     
...
# p<-ggforest(res.cox, data = Data1,  #???ݼ?
#          main = 'Hazard ratio',  #????
#          #cpositions = c(0.05, 0.15, 0.35),  #ǰ???о???
#          fontsize = 1, #??????С
#          noDigits = 3 #????HRֵ?Լ?95%CI??С??λ??
# )
# dev.off()
# 
# print(p)
...
# 
# 
# # res.cox<-coxph(formula = Surv(Data1$Followup_time_years,Data1$SPMS_conversion) ~ Data1$Group_subtype+Data1$Age+Data1$Sex._F1_M0, data = Data1)
# # 
# # 
# # ggsurvplot(data=Data1,survfit(res.cox),color="#2E9FDF",ggtheme=theme_minimal())
# # 
# # subtype_df<-within(Data1,{Group_subtype<-factor(Group_subtype,labels=c('A0','A1','A2'))})
# # 
# # res.cox<-coxph(formula = Surv(Followup_time_years,SPMS_conversion) ~ Group_subtype+Age+Sex._F1_M0, data = subtype_df)
# # 
# # 
# # fit<-survfit(res.cox,newdata=Data1)
# # # 
# # ggsurvplot(fit,subtype_df,conf.int=TRUE,legend.labs=c("Subtype=1","Subtype=2","Subtype=3"),ggtheme=theme_minimal())
# 
# # p2 = ggboxplot(Data, x = "Group_subtype", y = "Age",
# #                color = "Group_subtype", palette = "jco")+
# #   stat_compare_means(method='anova')
# # print(p2)
# # 
# # MS<-Data[Data$Group=='MS',]
# # AQP4Pos_NMOSD<-Data[Data$Group=='AQP4Pos_NMOSD',]
# # AQP4Neg_NMOSD<-Data[Data$Group=='AQP4Neg_NMOSD',]
# # MOGAD<-Data[Data$Group=='MOGAD',]
# 
