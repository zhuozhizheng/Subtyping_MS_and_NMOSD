...
savepath<-"D:/ZZZ/Manuscripts/Subtyping/Sustain_results/Results/MS_0411_Supp"
setwd(savepath)
...
Data=Data[Data$FA_cerebral_WM>0,]
unique(Data$Dataset)
...
for (i in 1:length(Ass_var))
{
  ...
  Data1<-Data1[Data1$Group_subtype!='MS-NA',]
  ...
  library(ppcor)
    ...
    # 
    # if(Ass_var[i] %in% Ass_var1)
    # {tem_data<-Data1[,c('ml_stage','tem_ass','EstimatedTotalIntraCranialVol')]}
    # 
    # if(Ass_var[i] %in% Ass_var2)
    # {tem_data<-Data1[,c('ml_stage','tem_ass','Education_years')]}
    ...
    {  
      ...
      colnames(Res_corr)[c(1,2)]<-paste0(c('_R','_P'))
      rownames(Res_corr)[i]<-Ass_var[i]
    }
 
}

write.csv(Res_corr,"All_var_correlation_all_group.csv")
...
pdf(file=paste0(LETTERS[i],'SPMS_conversion_Surrival_curve.pdf'))
p1<-ggsurvplot(fit,pval=TRUE,conf.int = FALSE,risk.table = TRUE,risk.table.y.text.col=TRUE,risk.table.col="strata",)
print(p1)

dev.off()

pdf(file=paste0(LETTERS[i],'SPMS_conversion_Cumulative_hazard.pdf'))
p1<-ggsurvplot(fit,pval=TRUE,conf.int = FALSE,risk.table = TRUE,risk.table.y.text.col=TRUE,risk.table.col="strata",fun="cumhaz")
print(p1)
dev.off()
...
pdf(file=paste0(LETTERS[i],'SPMS_conversion_ggforest_cox.pdf'))
p<-ggforest(res.cox, data = Data1,  #数据集
         main = 'Hazard ratio',  #标题
         #cpositions = c(0.05, 0.15, 0.35),  #前三列距离
         fontsize = 1, #字体大小
...
write.csv(ressult$coefficients,paste0(LETTERS[i],'SPMS_conversion_ggforest_cox.csv'))

topptx(figure = p,filename = paste0(LETTERS[i],"SPMS_conversion_ggforest_cox.pptx"))
