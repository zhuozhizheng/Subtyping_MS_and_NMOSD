...
savepath<-"D:/ZZZ/Manuscripts/Subtyping/Sustain_results/Results/AQP4_NMOSD_0411_Supp"
setwd(savepath)
...
for (i in 1:length(Ass_var))
{
    ...
    
  {
    ...
    Ass_var1<-c('choroid_volume')
    Ass_var2<-c('MMSE','MOCA','CVLT','BVLT','PASAT','SDMT','COWAT','Beton')
    
    tem_data<-Data1[Data1$Group_subtype==LETTERS[j],c('ml_stage','tem_ass')]
    
    if(Ass_var[i] %in% Ass_var1)
    {tem_data<-Data1[Data1$Group_subtype==LETTERS[j],c('ml_stage','tem_ass','EstimatedTotalIntraCranialVol')]}
    
    if(Ass_var[i] %in% Ass_var2)
    {tem_data<-Data1[Data1$Group_subtype==LETTERS[j],c('ml_stage','tem_ass','Education_years')]}
...
for (i in 1:length(Ass_var))
{
  ...
  pdf(file=paste0('All_subtype_group_Stage_assocaited_with_',Ass_var[i],'.pdf'),width=20,height=3)
  p1<-ggplot(data = Data1, aes(x =ml_stage , y = tem_ass),palette = "jco") +
    geom_smooth(method = "lm",size=2, se=FALSE) +
    #scale_color_manual(values=colors_2)+
    geom_point(size=1,
               alpha=0.3)+
  ...
  library(ppcor)
    ...
    tem_data<-Data1[,c('ml_stage','tem_ass')]
    ...
    {  
      ...
      Res_corr[i,1]<-cor_res$estimate[2,1]
      Res_corr[i,2]<-cor_res$p.value[2,1]
      colnames(Res_corr)[c(1,2)]<-paste0(LETTERS[j],c('_R','_P'))
      rownames(Res_corr)[i]<-Ass_var[i]
    }
...
write.csv(Res_corr,"All_var_correlationa_all_groups.csv")
...
for(i in 1:4) 
{
  ...
  Data1<-Data
  Data1$Group_subtype<-as.character(Data1$Group_subtype)
  Data1[Data$Group_subtype==LETTERS[i],'Group_subtype']<-LETTERS[i]
  Data1[Data$Group_subtype!=LETTERS[i],'Group_subtype']<-c('Group_ref');
  
  Data1$Group_subtype<-factor(Data1$Group_subtype,levels=c('Group_ref',LETTERS[i]))
...
pdf(file=paste0(LETTERS[i],'EDSS_Progress_Surrival_curve.pdf'))
p1<-ggsurvplot(fit,pval=TRUE,conf.int = FALSE,risk.table = TRUE,risk.table.y.text.col=TRUE,risk.table.col="strata",)
print(p1)

dev.off()

pdf(file=paste0(LETTERS[i],'EDSS_Progress_Cumulative_hazard.pdf'))
...
pdf(file=paste0(LETTERS[i],'EDSS_Progress_ggforest_cox.pdf'))
...
write.csv(ressult$coefficients,paste0(LETTERS[i],'EDSS_Progress_ggforest_cox.csv'))

topptx(figure = p,filename = paste0(LETTERS[i],"EDSS_Progress_ggforest_cox.pptx"))
}
...
pdf(file=paste0(LETTERS[i],'Followup_relapse.pdf'))
p1<-ggsurvplot(fit,pval=TRUE,conf.int = FALSE,risk.table = TRUE,risk.table.y.text.col=TRUE,risk.table.col="strata",)
print(p1)

dev.off()

pdf(file=paste0(LETTERS[i],'Followup_relapse_Cumulative_hazard.pdf'))
p1<-ggsurvplot(fit,pval=TRUE,conf.int = FALSE,risk.table = TRUE,risk.table.y.text.col=TRUE,risk.table.col="strata",fun="cumhaz")
print(p1)
dev.off()
...
pdf(file=paste0(LETTERS[i],'Followup_relapse_ggforest_cox.pdf'))
p<-ggforest(res.cox, data = Data1,  #数据集
         main = 'Hazard ratio',  #标题
         #cpositions = c(0.05, 0.15, 0.35),  #前三列距离
         fontsize = 1, #字体大小
...
write.csv(ressult$coefficients,paste0(LETTERS[i],'Followup_relapse_ggforest_cox.csv'))

topptx(figure = p,filename = paste0(LETTERS[i],"Followup_relapse_ggforest_cox.pptx"))
}
