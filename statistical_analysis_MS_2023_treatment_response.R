...
#write.csv(Data,'MS_subtype_data.csv');

Data=read.csv('D:/ZZZ/Manuscripts/Subtyping/Sustain_results/Results/MS_0411/MS_subtype_data.csv',header = TRUE);

Data<-Data[Data$Group=='MS',]

#生存曲线分析
setwd('D:/ZZZ/ManuscriptsSubtyping/Sustain_results/Results/MS_0411')
var1<-c('MS-NA','MS-C','MS-SC','MS-DGM')
...
for(i in var1)
{
  ...
  pdf(file=paste0(i,'_treatment_response_SPMS_conversion_Surrival_curve.pdf'),width = 8,height=8)
  p1<-ggsurvplot(fit,pval=TRUE,conf.int = FALSE,risk.table = TRUE,risk.table.y.text.col=TRUE,risk.table.col="strata",)
  print(p1)
  dev.off()
...
Data1$Group_subtype<-factor(Data1$Group_subtype,levels=c('MS-NA','MS-C','MS-SC','MS-DGM'))

Data1$Followup_relapse<-as.factor(Data1$Followup_relapse)

tem_Data1<-Data1[Data1$Treatment=='DMT',]
tem_Data1$Group_subtype<-factor(tem_Data1$Group_subtype,levels=c('MS-NA','MS-C','MS-SC','MS-DGM'))
...
write.csv(mytable,paste0('MS_count_Followup_relapse.csv'));

pdf(file=paste0('MS_count_Followup_relapse.pdf'),width = 5,height=3)
ggplot(tem_Data1, aes(x = Group_subtype,color=Followup_relapse)) +
  geom_bar(aes(fill = Followup_relapse),position = "stack",width=0.3) +
  theme(legend.position = "top")+ylim(0,40)+
  theme_classic()
...
write.csv(mytable,paste0('MS_count_EDSS_Progress.csv'));

pdf(file=paste0('MS_count_EDSS_Progress.pdf'),width = 5,height=3)
ggplot(tem_Data1, aes(x = Group_subtype,color=EDSS_Progress)) +
  geom_bar(aes(fill = EDSS_Progress),position = "stack",width=0.3) +
  theme(legend.position = "top")+ylim(0,75)+
  theme_classic()
dev.off()
